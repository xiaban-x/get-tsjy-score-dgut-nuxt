...
import { parseStudentsFromExcel, parseStudentsFromText, exportStudentsToCSV, downloadCSV } from '~/utils/parser'

/**
 * 学生数据导入导出处理组合函数
 */
export function useExportImport(studentsRef?: Ref<Student[]>) {
  // 导入状态
   ...
   * 从Excel文件导入学生数据
   */
  const importFromExcel = async (file: File): Promise<Student[]> => {
    if (!file) return []
  ...
  const exportToCSV = (students?: Student[]) => {
    // 使用提供的学生数据或直接使用引用的数据
    const studentsToExport = students || studentsRef?.value || []
    
    if (studentsToExport.length === 0) {
      importError.value = '没有学生数据可导出'
      return
    }
    
    try {
      const csvContent = exportStudentsToCSV(studentsToExport)
      downloadCSV(csvContent, '学生学分数据.csv')
    } catch (error) {
      importError.value = error instanceof Error ? error.message : '导出CSV失败'
    }
  }
